...
const handler = async ({
  ...
  const redwoodProjectPaths = (0, _lib.getPaths)();
  const redwoodProjectConfig = (0, _projectConfig.getConfig)();

  // Starting values of ports from config (redwood.toml)
  let apiPreferredPort = (0, _parseInt2.default)(redwoodProjectConfig.api.port);
  let webPreferredPort = (0, _parseInt2.default)(redwoodProjectConfig.web.port);
  ...
  const webCommand = redwoodProjectConfig.web.bundler === 'vite' // @NOTE: can't use enums, not TS
    ? `yarn cross-env NODE_ENV=development rw-vite-dev ${forward}` : `yarn cross-env NODE_ENV=development RWJS_WATCH_NODE_MODULES=${watchNodeModules ? '1' : ''} webpack serve --config "${webpackDevConfig}" ${forward}`;

  let apiCommand = ['yarn', 'cross-env', 'NODE_ENV=development', 'NODE_OPTIONS=--enable-source-maps', 'yarn', 'nodemon', '--quiet', `--watch "${redwoodConfigPath}"`, '--exec', `"${['yarn', 'rw-api-server-watch', `--port ${apiAvailablePort}`, `--host '::'`, getApiDebugFlag(), '|', 'rw-log-formatter'].join(' ')}"`].join(' ');

  if (nodeVite) {
    apiCommand = [
      "vite-node",
      "--watch api/src/server.ts",
    ].join(' ');
  }
  ...
  const jobs = {
    api: {
      ...
      command: apiCommand,
      prefixColor: 'cyan',
      runWhen: () => _fs.default.existsSync(redwoodProjectPaths.api.src)
    },
    web: {
      name: 'web',
      command: webCommand,
      ...
      runWhen: () => _fs.default.existsSync(redwoodProjectPaths.web.src)
    },
    gen: {
      name: 'gen',
