...
const jsDeprecationNotice = () => {
  ...
  const matches = _fastGlob.default.sync(['App.js', 'Routes.js', 'components/**/*.js', 'pages/**/*.js'], {
    cwd: (0, _lib.getPaths)().web.src,
    ignore: ['**/.*.js', '**/*.fixtures.js', '**/*.mock.js', '**/*.routeHooks.js', '**/*.test.js', '**/*.spec.js']
  });
  if (matches.length) {
    console.warn(_colors.default.warning(`DEPRECATION NOTICE: File extensions for JS files containing JSX must be named \`.jsx\`:\n\n  ${matches.join('\n  ')}\n\nSupport for \`.js\` files containing JSX will be dropped in Redwood 7.0.0.\nThere is a codemod available to update these extensions for you:\n\n  npx @redwoodjs/codemods convert-js-to-jsx\n\n* Hide this notice by setting the ENV variable REDWOOD_DISABLE_JS_DEPRECATION_NOTICE=1`));
  }
};
const handler = async ({
  side = ['api', 'web'],
  ...
  const webCommand = (0, _projectConfig.getConfig)().web.bundler !== 'webpack' // @NOTE: can't use enums, not TS
  ...
  let apiCommand = `yarn cross-env NODE_ENV=development NODE_OPTIONS=--enable-source-maps yarn nodemon --quiet --watch "${redwoodConfigPath}" --exec "yarn rw-api-server-watch --port ${apiAvailablePort} ${getApiDebugFlag()} | rw-log-formatter"`

  if (viteNode) {
    apiCommand = [
      "vite-node",
      "--watch api/src/server.mts",
    ].join(' ');
  }
